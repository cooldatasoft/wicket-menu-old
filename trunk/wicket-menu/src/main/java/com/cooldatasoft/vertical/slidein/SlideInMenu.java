...
/**
 * Configuring the menu To change the contents of the menu to your own, edit the
 * sitems[] array. You may now also specify a target for the links, if you wish
 * them to be loaded in a new window or frame.
 * 
 ...
 * 
 * border:1.5px solid green; background-color:lightyellow;
 * layer-background-color:lightyellow; font:bold 12px Verdana; line-height:20px;
 * 
 * The first line determines the appearance of the border that surrounds the
 * menu (change 1.5px and green as desired). The second and third line effects
 * the background color of the menu. The fourth line determines the font of the
 * menu (change 12px and Verdana as desired). The last line deterines the
 * spacing between each line of text in the menu.
 * 
...
public class SlideInMenu extends Panel implements IHeaderContributor {
	...
	private final static ResourceReference MENU_JS = new JavaScriptResourceReference(
			SlideInMenu.class, "js/SlideInMenu.js");
	private final static ResourceReference MENU_CSS = new CssResourceReference(
			SlideInMenu.class, "css/SlideInMenu.css");
			...
			if (menuItem.getDestinationType() == DestinationType.WEB_PAGE_CLASS) {
				response.getResponse().write(
						"sitems["
								...
								+ RequestCycle.get().urlFor(
										menuItem.getResponsePageClass(), null)
								+ "\"];\n");
				...
				response.getResponse().write(
						"sitems["
								...
								+ "\""
								+ RequestCycle.get().urlFor(
										menuItem.getResponsePage().getClass(),
										null) + "\"];\n");
			} else {
				System.err.println(menuItem.getDestinationType());
				throw new RuntimeException(
						"This menu can only be applied to wicket web pages");
			}
		}
